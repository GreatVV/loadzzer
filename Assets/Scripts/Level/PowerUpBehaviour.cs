...
public static class PowerUpBehaviour {
	...
	public static void ApplyPowerUp(this Gamefield gamefield, List<Chuzzle> tilesToKill,  Chuzzle chuzzle)
    {
        ...
        {
            var horizontalChuzzles = gamefield.chuzzles.Where(x => x.Current.y == chuzzle.Current.y && x.PowerType == PowerType.Usual);            
            tilesToKill.AddUniqRange(horizontalChuzzles);             
        }

        if (chuzzle.PowerType == PowerType.VerticalLine)
        {
            var vertical = gamefield.chuzzles.Where(x => x.Current.x == chuzzle.Current.x && x.PowerType == PowerType.Usual);
            tilesToKill.AddUniqRange(vertical);    
        }

        if (chuzzle.PowerType == PowerType.Bomb)
        {
            var square = gamefield.chuzzles.Where(x => (x.Current.x == chuzzle.Current.x - 1 || x.Current.x == chuzzle.Current.x + 1 || x.Current.x == chuzzle.Current.x) && (x.Current.y == chuzzle.Current.y - 1 || x.Current.y == chuzzle.Current.y || x.Current.y == chuzzle.Current.y + 1) && x.PowerType == PowerType.Usual);
